...
A curated, opinionated index of best-in-class libraries per domain.

Every pick is judged against the agent-stack default profile:

- **Structured JSON** output
- **OpenTelemetry** trace/span correlation
- **Low overhead** / high throughput
- **Stdout-first** (12-factor — the platform collects)
- Clean **ergonomics** / API
...
> **Adding an index?** Append a new `##` section here, judge candidates against the profile at the top, keep the pick/runner-up/caveat shape, and add a bullet to the [Index](#index). Link it from the README's *Awesome libraries* section.
